module eigen
    ...
    implicit none
    private 
    public lanczos,symetric_eigenvalues,matvecmul,LanczosTimeEvolution
...
contains
    ...
    subroutine LanczosTimeEvolution(BasisDim,GroundState,InitialState, Time,Evaluator,Evolution)
        integer,intent(in) :: BasisDim
        real(dp),intent(in) :: InitialState(BasisDim),GroundState(BasisDim)
        real(dp),intent(out) :: Evolution(:)
        ...
        real(dp),allocatable :: Tj(:,:), RitzValues(:),RitzVectors(:,:)
        real(dp),allocatable :: RitzNorms(:)
        real(dp) :: FinalState(BasisDim)
        !private variables
        real(dp) :: V(BasisDim,BasisDim) ! the matrix V that holds the orthonormal basis {v,Av,A^2v,...}
                               !, lanczos basis
        real(dp) :: r(BasisDim) ! aux vector for computation
        integer :: j, k
        real(dp) :: Beta(1:BasisDim),Alpha(1:BasisDim) !banded elements of the matrix

        real(dp),parameter :: eps = tiny(1.0_dp), thres = sqrt(eps)
        

        Beta(1) =  norm2(InitialState)
        V(:,1) = InitialState /Beta(1)
        ...
        do j=1,BasisDim-1
            ...
            if(j > 2) then
                ...
                call ComputeRitzNorms(j)
                if( RitzNorms(1) < 1e-8) then
                    if(allocated(D))deallocate(D)
                    print *, "Computing  time evolution"
                    print *, "Original basis: ", BasisDim
                    print *, "Lanczos basis: ", j
                    allocate(D(j,j))
                    do k=1,size(Time)
                        ! print *, Time(k)
                        call exponent(j,RitzValues,D,Time(k))

                        D= matmul(RitzVectors,D)
                        D = matmul(D, transpose(RitzVectors))
                        V =  matmul(V(:,1:j),D)
                        FinalState = V(:,1)
                        Evolution(k) = DOT_PRODUCT(FinalState,GroundState)
                    enddo
                    print *, "Computed evolution"
        ...
        enddo
        contains
        !give the eigenvalues, returns
        !the exponential matrix
        subroutine exponent(N,V,A,Time)
            integer,intent(in) :: N
            complex(dp),intent(out) :: A(N,N)
            real(dp),intent(in) :: V(N),Time
            
            integer :: i
            A = 0.0_dp
            do i=1,N
                A(i,i) = exp(-  (0.0_dp,1.0_dp)*V(i)*Time)
            enddo
